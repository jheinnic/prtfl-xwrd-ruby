...
class Experiment
  def initialize()
    ...
    @bitwords = @words.map {|w| MockLetterItem.word_to_bitfield(w) }
  end
  ...
  def reset_items() 
    puts @bitwords.inspect()
    word_items = @bitwords.map { |w| MockWordItem.new(w & @known_bitfield) }
  ...
  def check_bits_from_array()
    bit_slots = (0...26).to_a
    single_bits = bit_slots.map { |x| 1 << x }
    bit_slots.combination(@num_unknowns) { |next_combo|
      bit_mask = 0
      next_combo.each do |next_slot|
        bit_mask = bit_mask | single_bits[next_slot]
      end
    }
  end

  def check_bits_from_comp()
    bit_slots = (0...26).to_a
    bit_slots.combination(@num_unknowns) { |next_combo|
      bit_mask = 0
      next_combo.each do |next_slot|
        bit_mask = bit_mask | 1 << next_slot
      end
    }
  end
  
  def check_bits_from_twidle()
    BitsetTwidler.new(26, @num_unknowns).each do |next_combo| ; end
  end
  ...
  def useLetterDeltaWordForce()
    ...
    @unused_bitset.combination(@num_unknowns) { |next_combination|
      ...
      @bitwords.each {|word| pay_offset += 1 if ((word & coverage) == 0)}
  ...
  def run_check()
    ...
    Benchmark.bmbm(7) do |x|
      5.times { x.report('Check Bits From Array') { check_bits_from_twidle() } }
      5.times { x.report('Check Bits From Array') { check_bits_from_array() } }
      5.times { x.report('Check Compute Bits') { check_bits_from_comp() } }
    end
  end
  ...
  def run_test()
    time = Time.now
    Benchmark.bmbm(7) do |x|
      x.report('First Force Letters, Force Words: ')  { useLetterForceWordForce(); }
      x.report('First Delta Letters, Force Words: ')  { useLetterDeltaWordForce(); }
      x.report('First Delta Letters, Naive Words: ')  { useLetterDeltaWordNaive(); }
      reset_items();
      x.report('First Delta Letters, Delta Words: ')  { useLetterDeltaWordDelta(); }
    
      later = Time.now
      puts "Time elapsed #{(later-time)*1000} milliseconds"
      time = later
    
      x.report('Second Force Letters, Force Words: ')  { useLetterForceWordForce(); }
      x.report('Third Force Letters, Force Words: ')  { useLetterForceWordForce(); }
      later = Time.now
      puts "Time elapsed #{(later-time)*1000} milliseconds"
      time = later

      x.report('Second Delta Letters, Force Words: ')  { useLetterDeltaWordForce(); }
      x.report('Third Delta Letters, Force Words: ')  { useLetterDeltaWordForce(); }
      later = Time.now
      puts "Time elapsed #{(later-time)*1000} milliseconds"
      reset_items();
      time = later

      x.report('Second Delta Letters, Naive Words: ')  { useLetterDeltaWordNaive(); }
      reset_items();
      x.report('Third Delta Letters, Naive Words: ')  { useLetterDeltaWordNaive(); }
      later = Time.now
      puts "Time elapsed #{(later-time)*1000} milliseconds"
      reset_items();
      time = later

      x.report('Second Delta Letters, Delta Words: ')  { useLetterDeltaWordDelta(); }
      reset_items();
      x.report('Third Delta Letters, Delta Words: ')  { useLetterDeltaWordDelta(); }
      later = Time.now
      puts "Time elapsed #{(later-time)*1000} milliseconds"
      reset_items();
      time = later
...
puts "Setting revealed"
#demo.change_revealed( 'yuehwpivjzrs' ) 
demo.change_revealed( '' ) 
